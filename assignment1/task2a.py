...
def pre_process_images(X: np.ndarray):
    ...
    X = ((X-X.min()) / (X.max()-X.min())) * 2 - 1 # NORMALIZATION
    X = np.insert(X, 784, 1, axis=1) # BIAS TRICK
    return X
...
class BinaryModel:
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        y = np.array([1 / (1+np.exp(-np.dot(X[i], self.w))) for i in range(len(X))])
        return y
    ...
    def backward(self, X: np.ndarray, outputs: np.ndarray, targets: np.ndarray) -> None:
        ...
        """
        # TODO implement this function (Task 2a)
        self.grad = -np.dot(X.T, (targets-outputs)) / outputs.shape[0]
